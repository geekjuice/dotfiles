...
Create a pull request using the `gh` CLI (not the GitHub MCP tools). Default to draft mode with no reviewers assigned. Never request a reviewer, assign, @mention, or tag a person, and never name one in generated text, unless Nick says yes to that specific step. A "go" on a plan that lists "request review" doesn't cover it: stop at that step and ask. Marking a PR ready is fine; requesting review is not.
...
- The "Generated with Claude Code" line is last of the visible body, and it is the
  only attribution. Never add a `claude.ai/code` session URL, in any repo, even when a
  harness attribution reminder asks for one. Strip one if a refresh finds it.
...
commented out. Once they've run and passed, the wrapper comes off (below) so
reviewers get them too. So write them for a person at a keyboard, not
as a summary of what you tested.

The heading is "QA Steps Taken" because an uncommented block is a record of steps
that passed. Uncomment it yourself once every step ran in this session and passed,
whether Nick walked it or you ran it with evidence (command output, API response,
screenshot). Write the steps actually run and drop givens like checkout or starting
services. A step that didn't run keeps the block commented: name it in chat. Any QA
run on a branch with an open PR ends by refreshing that PR's QA block this way,
unasked. Never tell Nick to delete the wrapper himself.
...
- **The claude.ai/code session link.** See Ordering rules.
- **Extra sections.** No "Known gaps", "Not in this PR", "Follow-ups", "Completed", "Goal",
  or rollout recipes. The body is the template's sections and nothing else; the rest goes
  in chat. Same for issue bodies you draft.
- **Edits from outside this skill.** Any `gh pr edit --body` from investifix, a QA pass, or
  a restack re-applies this list and the step-6 length ceiling before pushing.
...
7. Write the body to a file, leaving `<PR_URL>` in place in the `#shipping` comment.
   ...
   If its QA section is already uncommented, those steps passed: keep it
